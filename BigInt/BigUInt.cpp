...
namespace big {
    ...
    BigUInt &BigUInt::operator*=(const BigUInt &rhs) {
        ...
        if (rhs.digitCount() == 1ul && rhs.mostSignificantDigit() == 0ul) {
            m_digits = {0ul};
        } else {
            if (this == &rhs) {
                square();
    ...
    size_t BigUInt::value() const {
        switch (digitCount()) {
            ...
            case 2:
                return s_base * mostSignificantDigit() + leastSignificantDigit();
            default:
                assert(false);
        }
    }
    ...
    void BigUInt::multiplyBySingleDigit(const size_t digit) {
        ...
        switch (digitCount()) {
            case 1: {
                ...
                m_digits = val > s_maxDigit ? std::vector{val % s_base, val / s_base} : std::vector{val};
            }
                break;
            default:
                resize(digitCount() + 1ul);
...
#define REPEAT_THREE(code) code code code
            REPEAT_THREE(
                    s0 = *resultIt + *rhsIt * multiplier + (s0 / s_base); *resultIt = s0 % s_base; ++resultIt; ++rhsIt;)
#undef REPEAT_THREE
            carry = s0 / s_base;
        }
        switch (rhsEnd - rhsIt) {

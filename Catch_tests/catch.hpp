...
namespace Catch {
    ...
    // Import begin/ end from std here so they are considered alongside the fallback (...) overloads in this
    // namespace
    using std::begin;
    using std::end;
...
namespace Catch {
    ...
    // Specialised comparison functions to handle equality comparisons between ints and pointers (NULL deduces
    // as an int)
    template <typename LhsT, typename RhsT>
...
namespace Catch {
    ...
    private:
        ...
        void
        printSummaryRow(std::string const &label, std::vector<SummaryColumn> const &cols, std::size_t row);
...
namespace Catch {
    ...
    void
    TagAliasRegistry::add(std::string const &alias, std::string const &tag, SourceLineInfo const &lineInfo) {
...
#define CATCH_FAIL(...)                                                                                      \
    INTERNAL_CATCH_MSG(                                                                                      \
        "CATCH_FAIL", Catch::ResultWas::ExplicitFailure, Catch::ResultDisposition::Normal, __VA_ARGS__)
